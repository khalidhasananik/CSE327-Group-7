                ...
                <form method="post" action=<?php echo 'query.php?member_id='.$sid_row['mid']?>>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="editMemberName" name="editMemberName"
                            value="<?php echo $sid_row['mname'];?>" placeholder="member Name" required>
                    </div>
                    <!-- Email input -->
                    <div class="mb-3">
                        <label for="editMemberEmail" class="form-label">Email</label>
                        <input type="email" class="form-control" id="editMemberEmail" name="editMemberEmail"
                            value="<?php echo $sid_row['email'];?>" placeholder="member Email" required>
                    </div>
                    <!-- Phone input -->
                    <div class="mb-3">
                        <label for="editMemberPhone" class="form-label">Phone</label>
                        <input type="tel" disabled class="form-control" id="editMemberPhone" name="editMemberPhone"
                            value="<?php echo $sid_row['phone'];?>" placeholder="member Phone number" required>
                    </div>
                    <!-- Address input -->
                    <div class="mb-3">
                        <label for="editMemberJoinDate" class="form-label">Join Date</label>
                        <input type="date" class="form-control" id="editMemberJoinDate" name="editMemberJoinDate"
                            placeholder="" value="<?php echo date('Y-m-d', strtotime($sid_row['joindate']));?>"
                            required>
                    </div>
                    <div class="mb-3">
                        <label for="editMemberPlan" class="form-label">Plan Subscribed</label>
                        <select class="form-control" name="editMemberPlan" id="editMemberPlan"
                            value="<?php echo $sid_row['pid'];?>" required>
                            <?php while ($row = mysqli_fetch_assoc($plan_query)) {?>
                            <option value="<?=$row['pid']?>" <?=  $sid_row['pid'] == $row['pid'] ? 'selected' : '' ?>>
                    ...
                    <!-- Submit button -->
                    <button type="submit" id="update-member-btn" name="update-member-btn" class="btn btn-primary">Update
                        Member</button>
                    <!-- End form -->
                </form>
                <!-- End form container -->
