<?php
    ...
    if (isset($_POST['member-search-btn'])) {
        ...
        header("location:$baseURL/admin/reservations/index.php?search=$search");
    }

    if ($_GET['search']) {
        $search = $_GET['search'];
        $select = "SELECT reservation.*, services.s_name as serviceName FROM `reservation` LEFT JOIN services ON reservation.sid = services.sid WHERE Name LIKE '$search%' OR Email LIKE '$search%' OR Phone LIKE '$search%' OR Site LIKE '$search%' OR s_name LIKE '$search%'";
        $s_query = mysqli_query($conn, $select);
    } else {
        $select = "SELECT reservation.*, services.s_name as serviceName FROM `reservation` LEFT JOIN services ON reservation.sid = services.sid";
        $s_query = mysqli_query($conn, $select);
    }
